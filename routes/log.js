...
//    console.log(req.session.user.userData);
    console.log("RE-RENDERING ", req.session.user.userData);
...
router.post('/Update', (req, res) => {
    console.log("--------------UPDATE---------------\n\n")
    let newUserData = req.body.draggedData; //the data with the dragged element removed
    let newEntry = req.body.updatedEntry; //the data of the dragged element at its new position
    console.log("newUserData  ", newUserData);
    console.log("newEntry  ", newEntry);

    console.log("new entry? == null ", (newEntry==null));
    console.log("new entry? === null ", (newEntry===null));
    console.log("new entry? == undef ", (newEntry==undefined));
    console.log("new entry? === undef ", (newEntry===undefined));

    console.log("new data? == null ", (newUserData==null));
    console.log("new data? === null ", (newUserData===null));
    console.log("new data? == undef ", (newUserData==undefined));
    console.log("new data? === undef ", (newUserData===undefined));
    ...
    if (newUserData === undefined) {
        ...
        if ((newEntry == undefined)) {  //the user removed the dragged element
            req.session.user.userData.splice(0, req.session.user.userData.length); //remove all of the user's data
            console.log("new entry does not exist, remove eveything ", req.session.user.userData);
        }
        else { //the user has removed the dragged element by dragging it past midnight
            req.session.user.userData.splice(0, req.session.user.userData.length); //remove all of the user's data
            req.session.user.userData.push(newEntry); //push the new entry
            console.log("new entry is not null, it exists push it ", req.session.user.userData);
        }
    }
    else { 
        //must be in double (()) for some reason to register??
        if ((newEntry === undefined)) { //the user removed the dragged element
            ...
            req.session.user.userData = newUserData; //user's data is the data with dragged element removed
            console.log("user's data is the data with dragged element removed ", req.session.user.userData);
        }
        else {
            req.session.user.userData = placeNewEntryInOrder(newUserData, newEntry);
            console.log("placing new entry in order ,", req.session.user.userData);
        }
    }

    console.log("CALLED b4 save?");
    // res.redirect('/log'); 
    // res.send(req.session.user.userData);
    ...
    //     }
    // });
    console.log("CALLED a4 save?");
    // res.redirect(req.get('referer')); 
    res.redirect('/log'); 
    

    console.log("CALLED log redir?");
    ...
    // }
    // else{
    //     console.log("current session data " , req.session.user.userData);
    //     req.session.user.userData.splice(0, req.session.user.userData.length);
